...
async def get_user_account_data(request: Request, response: Response):
    try:
        ...
        user_lending_data = lendingService.get_user_lending_data()
...
async def get_user_config(request: Request, response: Response):
    try:
        # if hasattr(request.state, "error"):
        #     raise request.state.error
        # user: User = request.state.user
        # request.app.logger.info(f"getting user lending pool data for - {user.id}")
        user_lending_data = lendingService.get_user_config()
        request.app.logger.info("done getting user lending pool data ")
        return responseService.send_response(
            response,
            status.HTTP_200_OK,
